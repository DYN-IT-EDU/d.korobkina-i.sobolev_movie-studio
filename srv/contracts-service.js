...
class ContractsService extends cds.ApplicationService {
  ...
  async appendContracts(req) {
    ...
    if (result) {
      return {
        success: true,
        message: `Contract with id ${newContract.ID} added successfully`,
        ID: newContract.ID,
      };
    } else {
  ...
  async deleteFromContracts(req) {
    ...
    if (result) {
      return {
        success: true,
        message: `Contract with ID ${ID} has been removed.`,
      };
    } else {
      return {
        success: false,
        message: `Failed to remove contract with ID ${ID}.`,
      };
    }
  }
  ...
  async checkExistense(req) {
    ...
    if (req.event === "addContract") {
      ...
      if (existingContract) {
        req.error(
          400,
          `Contract already exists for MovieID ${movieID}, PersonID ${personID}, and RoleID ${roleID}.`
        );
      }
    }
  ...
  async appendDetails(data) {
    const ID = data.ID;
    const { Contracts } = this.entities;
    const currentContract = await SELECT.one(Contracts, ["details"]).where({
      ID,
    });
